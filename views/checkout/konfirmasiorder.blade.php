...
<div class="inner-column row padd">
        <divclass="inner-bg col-lg-12 col-xs-12 col-sm-8">
		...
		<div class="row">
			...
			@if($order->jenisPembayaran==1)
				...
				<div class="form-group">
				  ...
				  <input type="text" class="md-input col-xs-12" placeholder="Nama Pengirim" name='nama' required>
				</div>
				<div class="form-group">
				  <label  class="control-label"> No Rekening:</label>
				  <input type="text" class="md-input col-xs-12" placeholder="No Rekening" name='noRekPengirim' required>
				</div>
				<div class="form-group">
				  <label  class="control-label"> Rekening Tujuan:</label>
				  <select name='bank' class="md-input col-xs-12">
				...
				<div class="form-group">
				  ...
				  @if($checkouttype==1)        
					<input type="text" class="md-input col-xs-12" placeholder="jumlah yg terbayar" name='jumlah' value='{{$order->status==0 ? $order->total : ""}}' required>
				  @else
					@if($order->status < 2)
					  <input class="md-input col-xs-12" placeholder="Jumlah pembayaran" type="text" name='jumlah' value='{{$order->dp}}' required>
					@elseif(($order->status > 1 && $order->status < 4) || $order->status==7)
					  <input class="md-input col-xs-12" placeholder="Jumlah pembayaran" type="text" name='jumlah' value='{{$order->total - $order->dp}}' required>
					@endif
				  @endif
				</div>
				<button type="submit" class="md-button narrow">Konfirmasi Order</button>
				{{Form::close()}}
			@endif
			</div>
		</div>
